    ...
    for name_to_test in modelsToTest:
        ...
        for index,(img, adv_img, mask,rect,target_patch, logo) in enumerate(iter(dataloader)):
            ...
            with torch.no_grad():
                ...
                # assert cv2.imwrite('inpainting/ensemble/'+name_to_test+'_original_place2.jpg',(original_output[name_to_test])[0].detach().cpu().numpy().transpose(1,2,0)*255)
                # assert cv2.imwrite('inpainting/ensemble/'+name_to_test+'_attacked_place2.jpg',(attacked_output[name_to_test])[0].detach().cpu().numpy().transpose(1,2,0)*255)
                # exit()
            ...
            #     # assert cv2.imwrite('inpainting/ensemble/target_img.jpg',target_img[0,...].detach().cpu().numpy().transpose(1,2,0)*255)
            #     # exit()
            ...
            for i in range(attacked_output[list(attacked_output.keys())[0]].size()[0]):
                ...
                for name in attacked_output.keys():
                    ...
                    assert cv2.imwrite(os.path.join('inpainting/ensemble/experiment_result/',opt.output_dir,name_to_test,'result/attacked_pred_mask_'+name+'_'+str(preprocess_num)+'.png'),attacked_mask_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    assert cv2.imwrite(os.path.join('inpainting/ensemble/experiment_result/', opt.output_dir,name_to_test,'result/original_pred_mask_'+name+'_'+str(preprocess_num)+'.png'),original_mask_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])

                    assert cv2.imwrite(os.path.join('inpainting/ensemble/experiment_result/',opt.output_dir,name_to_test,'result/final_attacked_'+name+'_'+str(preprocess_num)+'.png'),final_attacked_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    assert cv2.imwrite(os.path.join('inpainting/ensemble/experiment_result/', opt.output_dir,name_to_test,'result/final_original_'+name+'_'+str(preprocess_num)+'.png'),final_original_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    # exit()
                    ...
                    # assert cv2.imwrite('inpainting/ensemble/edge_example.jpg',edge[0].cpu().numpy().transpose(1,2,0)*255)
                    # exit()
                    if mask!=None:
                    ...
                    assert cv2.imwrite(os.path.join('inpainting/ensemble/experiment_result/',opt.output_dir,name_to_test,'result/attacked_mask_'+name+'_'+str(preprocess_num)+'.png'),attacked_mask_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    # assert cv2.imwrite(os.path.join('inpainting/ensemble/experiment_result/', opt.output_dir,name_to_test,'result/original_mask_'+name+'_'+str(preprocess_num)+'.png'),original_mask_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])

                    assert cv2.imwrite(os.path.join('inpainting/ensemble/experiment_result/',opt.output_dir,name_to_test,'result/final_attacked_'+name+'_'+str(preprocess_num)+'.png'),final_attacked_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    # assert cv2.imwrite(os.path.join('inpainting/ensemble/experiment_result/', opt.output_dir,name_to_test,'result/final_original_'+name+'_'+str(preprocess_num)+'.png'),final_original_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    # assert cv2.imwrite('inpainting/ensemble/experiment_result/'+opt.output_dir+'/attacked_input/'+name+'_'+str(preprocess_num)+'.png',input_with_final_noise[i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    if rect.dim() == 3:
